...
def compose_cvoc_config(filenames, output_file='CVocConf.json'):
    ...
    for filename in filenames:
        ...
        # Check if file exists, if not, try searching in services/
        if not os.path.exists(full_filename):
            scripts_dir = os.path.dirname(os.path.abspath(__file__))
            services_dir = os.path.join(os.path.dirname(scripts_dir), 'services')
            if os.path.exists(services_dir):
                found = False
                for service in os.listdir(services_dir):
                    candidate = os.path.join(services_dir, service, 'configs', basename)
                    if os.path.exists(candidate):
                        full_filename = candidate
                        found = True
                        break
                if not found:
                    print(f"Warning: File {basename} not found in any service configs, skipping...", file=sys.stderr)
                    continue
            else:
