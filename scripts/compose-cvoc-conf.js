...
function composeCvocConfig(filenames, outputFile = 'CVocConf.json') {
    ...
    for (const filename of filenames) {
        ...
        let fullFilename = dir ? path.join(dir, nameWithExt) : nameWithExt;
        ...
        if (!fs.existsSync(fullFilename)) {
            ...
            if (fs.existsSync(servicesDir)) {
                ...
                for (const service of services) {
                    const candidate = path.join(servicesDir, service, 'configs', nameWithExt);
                ...
                if (!found) {
                    console.warn(`Warning: File ${nameWithExt} not found in any service configs, skipping...`);
                    continue;
                }
            } else {
